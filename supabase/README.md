...
| # | Plik | Etap §9 | Warunki wejścia | Stan warunków (aktualizacja 2026-08-09) |
|---|---|---|---|---|
| 0 | `snapshot/snapshot_queries.sql` → wypełnienie `restore_snapshot.sql` | 0 | jawna zgoda na SELECT-y na live | **MATERIALIZED 2026-08-11** — Q1-Q9 wykonane read-only (zgoda C4) na `ezemaacyyvbpjlagchds`; `supabase/snapshot/restore_snapshot.sql` zmaterializowany ze wszystkich 9 wyników (Sekcje A-F wypełnione, zero placeholderów). Zero SQL wykonanego POZA Q1-Q9 (sam plik restore nie uruchomiony). |
| 1 | `20260808000100_stage06_orders_additive_columns.sql` | 6 | etap 0 | gotowe po etapie 0 |
| 2 | `20260808000200_stage08_runtime_log_config_denyall.sql` | 8 | etap 0; poprawka `sessionAdapter` — klasyfikacja odmowy RLS (42501) do memory-fallback (§7 planu, zadanie D2) | **ZWERYFIKOWANE (review T8): klienci spoza write-setu T2 (`server.js:28-39`, `api/brain/supabaseClient.js:26-36`) używają wyłącznie `SUPABASE_SERVICE_ROLE_KEY`, fail-fast — zdjęte jako blokada.** Blokada `sessionAdapter` (D2) — **ZAMKNIĘTA 2026-08-09, commit `d42df6a`**: klasyfikacja odmowy uprawnień (`42501` / `permission denied` / `row-level security policy`) degraduje do memory-fallback w `loadFromSupabase`/`saveToSupabase`/`touchInSupabase`; dual-schema probing, kontrakt sesji i `sessionStore.js` bez zmian; test `api/brain/tests/sessionAdapter.permissionDenied.test.js` 8/8, baseline 49/49 bez regresji; review Opusa 2026-08-09 read-only: **D2 CLOSED**. Pozostały warunek wejścia: etap 0 |
