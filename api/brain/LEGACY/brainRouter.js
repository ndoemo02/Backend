...
const USE_BRAIN_V2 = process.env.BRAIN_V2 === 'true' || true; // FORCE V2 ON
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (shadowText && v2Pipeline) {
      v2Promise = v2Pipeline.process(shadowId, shadowText, { shadow: true })
        .catch(err => {
...
Spróbuj wybrać inną restaurację (np. numer lub nazwę).`;
    ...
    // Filter restaurants for response (don't send list if not needed)
    const shouldSendRestaurants = ['find_nearby', 'show_city_results', 'change_restaurant', 'show_more_options', 'recommend', 'select_restaurant'].includes(intent);
    const restaurantsToSend = shouldSendRestaurants ? (restaurants || []) : [];
    ...
    const finalResponse = {
      ...
      // Only send restaurants list if the intent is related to discovery/selection
      restaurants: restaurantsToSend,
      menuItems: menuItems || [],
    ...
    if (restaurantsToSend?.length) {
      finalResponse.locationRestaurants = restaurantsToSend;
    }
